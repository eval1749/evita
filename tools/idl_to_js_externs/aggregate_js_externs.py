...
def main(argv):
    if len(argv) != 7:
        raise Exception('Usage: %s output_path header_js_path js_externs_directory idl_list_path static_directory static_list_path' % os.path.basename(argv[0]))
    ...
    names = sorted([os.path.splitext(os.path.basename(idl_file_name))[0]
