...
class IoDelegateImpl::IoHandler : public base::MessagePumpForIO::IOHandler {
  ...
  public: void ReadFile(HANDLE file_handle, uint8_t* buffer, size_t num_read,
                       const IoDelegate::FileIoCallback& callback);
  public: void WriteFile(HANDLE file_handle, uint8_t* buffer, size_t num_write,
                         const IoDelegate::FileIoCallback& callback);
...
void IoDelegateImpl::IoHandler::ReadFile(HANDLE file_handle, uint8_t* buffer,
    size_t num_read, const domapi::IoDelegate::FileIoCallback& callback) {
  auto const context = CreateIoContext(callback);
  DWORD read;
  ::ReadFile(file_handle, buffer, num_read, &read, &context->overlapped);
}

void IoDelegateImpl::IoHandler::WriteFile(HANDLE file_handle, uint8_t* buffer,
    size_t num_write, const domapi::IoDelegate::FileIoCallback& callback) {
  auto const context = CreateIoContext(callback);
  DWORD written;
  ::WriteFile(file_handle, buffer, num_write, &written, &context->overlapped);
}
...
void IoDelegateImpl::IoHandler::OnIOCompleted(IOContext* context,
  ...
  static_cast<IoContextImpl*>(context)->callback.Run(
      bytes_transfered, static_cast<int>(error));
  delete context;
}
...
void IoDelegateImpl::ReadFile(domapi::IoHandle* io_handle, uint8_t* buffer,
                              size_t num_read,
...
void IoDelegateImpl::WriteFile(domapi::IoHandle* io_handle, uint8_t* buffer,
                               size_t num_write,
                               const FileIoCallback& callback) {
