...
class BeginRecord : public Record
{
    ...
    // ctor
    public: BeginRecord(const char16* s) : m_pwszName(s) {}
    ...
    public: virtual void Undo(Buffer* pBuffer)
    {
        ...
        #endif // DEBUG_UNDO
        pBuffer->GetUndo()->RecordEnd(m_pwszName);
    } // Undo
}; // BeginRecord
...
class EndRecord : public Record
{
    ...
    // ctor
    public: EndRecord(const char16* s) : m_pwszName(s) {}

    // [I]
    public: bool CanMerge(const char16* pwszName)
    {
        return m_pwszName == pwszName && '*' == *pwszName;
    } // IsSameGroup
    ...
    public: virtual void Undo(Buffer* pBuffer)
    {
        ...
        #endif // DEBUG_UNDO
        pBuffer->GetUndo()->RecordBegin(m_pwszName);
    } // Undo
}; // EndRecord
...
//
void UndoManager::RecordBegin(const char16* pwszName)
{
    ...
    BeginRecord* pRecord = new(this) BeginRecord(pwszName);
    addRecord(pRecord);
...
//
void UndoManager::RecordEnd(const char16* pwszName)
{
    if (m_eState == State_Disabled) return;
...
// UnboBlock::UndoBlock dtor
UndoBlock::~UndoBlock()
{
    m_pUndo->CheckPoint();
    m_pUndo->RecordEnd(m_pwszName);
} // ~UndoBlock
...
// UndoBlock::init
//
void UndoBlock::init(Buffer* p, const char16* s)
{
    m_pwszName = s;
    m_pUndo = p->GetUndo();
    m_pUndo->CheckPoint();
