...
void StdOutPrintf(const char* pszFormat, ...) {
  ...
  if (::IsDebuggerPresent()) {
    ...
    DWORD cbWritten;
    ::WriteFile(::GetStdHandle(STD_OUTPUT_HANDLE), szBuf, ::lstrlenA(szBuf),
                &cbWritten, 0);
  }
}
...
class PosnStack final {
 public:
  explicit PosnStack(int const capacity)
      : capacity_(capacity), count_(0), elements_(new Posn[capacity]) {}
  ...
  void EnsureCapacity(int const size) {
...
class Engine : public Regex::SourceInfo {
 ...
 public:
  Engine(IMatchContext* pIContext,
         const int* prgnCode,
 ...
 protected:
  Engine() : control_stack_(ControlStackSize), value_stack_(ValueStackSize) {}

  bool execute() { return execute(m_lPosn); }
  bool execute(Posn p) { return execute(p, p); }
  bool execute(Posn, Posn);

  bool m_fBackward;
  /// <summary>
  /// Limit source position. Used for detecting super-linear situation.
  /// </summary>
  Posn m_lLoopLimitPosn;
  Posn m_lMatchEnd;
  Count m_lMinLen;
  Posn m_lPosn;
  Posn m_lScanStop;
  int m_nCxp;
  int m_nPc;
  IMatchContext* m_pIContext;
  const int* m_prgnCode;
  PosnStack control_stack_;
  PosnStack value_stack_;
 ...
 private:
  ...
  static bool charEqCs(char16 a, char16 b) { return a == b; }

  static bool charSetEq(char16 a, const StringOperand* s) {
    for (StringOperand::Enum oEnum(s); !oEnum.AtEnd(); oEnum.Next()) {
      if (oEnum.Get() == a)
        return true;
    }
    return false;
  ...
  Posn cpop() { return control_stack_.Pop(); }

  void cpush(Control eCode) {
    control_stack_.EnsureCapacity(1);
    control_stack_.Push(eCode);
  }

  void cpush(Control eCode, Posn a) {
    control_stack_.EnsureCapacity(2);
    control_stack_.Push(a);
    control_stack_.Push(eCode);
  }

  void cpush(Control eCode, Posn a, Posn b) {
  ...
  void cpush(Control eCode, Posn a, Posn b, Posn c) {
  ...
  bool execute1();
  int fetchCapture(int k) const { return fetchInt(k); }
  Op fetchOp() const { return static_cast<Op>(fetchPosn(0)); }
  char16 fetchChar(int k) const { return static_cast<char16>(fetchPosn(k)); }
  Count fetchCount(int k) const { return fetchPosn(k); }
  int fetchInt(int k) const { return static_cast<int>(fetchPosn(k)); }
  Posn fetchPosn(int k) const { return m_prgnCode[m_nPc + k]; }
  int fetchPc(int k) const { return static_cast<int>(fetchPosn(k)); }

  const StringOperand* fetchString(int k) const {
    auto const ofs = fetchCount(k);
    return reinterpret_cast<StringOperand*>(reinterpret_cast<Int>(m_prgnCode) +
                                            ofs);
  }

  char16 getChar() const { return m_pIContext->GetChar(m_lPosn); }
  bool isBackward() const { return m_fBackward; }
  ...
  bool isAsciiWordBoundary() const {
    if (m_lPosn == m_lStart)
      return true;
  ...
  bool isCharEq_Ci() const { return charEqCi(getChar(), fetchChar(1)); }
  bool isCharEq_Cs() const { return getChar() == fetchChar(1); }
  bool isCharSetEq() const { return charSetEq(getChar(), fetchString(1)); }

  bool isAsciiDigitCharEq() const {
    return m_pIContext->IsAsciiDigitChar(getChar());
  }

  bool isAsciiSpaceCharEq() const {
    return m_pIContext->IsAsciiSpaceChar(getChar());
  }

  bool isAsciiWordCharEq() const {
  ...
  bool isUnicodeWordBoundary() const {
    if (m_lPosn == m_lStart)
      return true;
  ...
  bool isUnicodeDigitCharEq() const {
    return m_pIContext->IsUnicodeDigitChar(getChar());
  }

  bool isUnicodeSpaceCharEq() const {
    return m_pIContext->IsUnicodeSpaceChar(getChar());
  }

  bool isUnicodeWordCharEq() const {
    return m_pIContext->IsUnicodeWordChar(getChar());
  }

#define DEFUN_NE(name) \
  bool is##name##Ne() const { return !is##name##Eq(); }

#define DEFUN_NE_CS(name)                                     \
  bool is##name##Ne_Ci() const { return !is##name##Eq_Ci(); } \
  bool is##name##Ne_Cs() const { return !is##name##Eq_Cs(); }
  ...
  void makeCapturesUnbound() { m_pIContext->ResetCaptures(); }

  bool rangeEqCi(char16 a, char16 b, char16 c) const {
    auto const u = m_pIContext->CharUpcase(b);
    return a <= u && u <= c;
  }

  static bool rangeEqCs(char16 a, char16 b, char16 c) {
    return a <= b && b <= c;
  }

  bool stringEqCi(Posn lStart, Posn lEnd, const StringOperand* pString) const {
    if (lEnd - lStart != pString->GetLength())
      return false;

    for (Posn lPosn = lStart; lPosn < lEnd; lPosn++) {
      auto const fEq =
          charEqCi(m_pIContext->GetChar(lPosn), pString->Get(lPosn - lStart));
      if (!fEq)
        return false;
    }
    return true;
  ...
  bool stringEqCs(Posn lStart, Posn lEnd, const StringOperand* pString) const {
    if (lEnd - lStart != pString->GetLength())
      return false;
    for (Posn lPosn = lStart; lPosn < lEnd; lPosn++) {
      auto const fEq =
          charEqCs(m_pIContext->GetChar(lPosn), pString->Get(lPosn - lStart));
      if (!fEq)
        return false;
    }
    return true;
  ...
  Posn vpop() { return value_stack_.Pop(); }

  void vpush(Posn const value) {
  ...
  char16 debugGetChar(Posn lPosn) const {
    ...
    if (lPosn >= m_lEnd)
      return '_';
    auto const wch = static_cast<char16>(m_pIContext->GetChar(lPosn) & 0xFF);
    if (wch < 0x20)
      return '.';
    if (wch > 0x7E)
      return '.';
...
void RegexObj::Describe() const {
...
#if 0
  {
    ...
    char* pszFooter = "";
    for (EnumCapture oEnum(this); !oEnum.AtEnd(); oEnum.Next()) {
...
void Engine::printControl() const {
  ...
  switch (p->m_eFormat) {
    case OpFormat_nth_start_end:
      StdOutPrintf(" nth=%d start=%d %d", control_stack_.top(1),
                   control_stack_.top(2), control_stack_.top(3));
      break;

    case OpFormat_nextPc_posn:
      StdOutPrintf(" nextPc=%d posn=%d", control_stack_.top(1),
    ...
    case OpFormat_nextPc_posn_minPosn:
      StdOutPrintf(" nextPc=%d posn=%d minPosn=%d", control_stack_.top(1),
                   control_stack_.top(2), control_stack_.top(3));
      break;

    case OpFormat_nextPc_posn_maxPosn:
      StdOutPrintf(" nextPc=%d posn=%d maxPosn=%d", control_stack_.top(1),
                   control_stack_.top(2), control_stack_.top(3));
      break;

    case OpFormat_cxp_vsp:
      StdOutPrintf(" cxp=%d vsp=%d", control_stack_.top(1),
                   control_stack_.top(2));
      break;
...
bool Engine::dispatch() {
  for (;;) {
    ...
    {
      StdOutPrintf("<");
      for (Posn lPosn = m_lPosn - 7; lPosn < m_lPosn; lPosn += 1) {
        StdOutPrintf("%c", debugGetChar(lPosn));
      }

      StdOutPrintf(">%c<", debugGetChar(m_lPosn));

      for (Posn lPosn = m_lPosn + 1; lPosn < m_lPosn + 7; lPosn += 1) {
        StdOutPrintf("%c", debugGetChar(lPosn));
      }
      StdOutPrintf("> ");

      StdOutPrintf("csp=%03d vsp=%03d pos=%03d|%c| ", control_stack_.count(),
                   value_stack_.count(), m_lPosn, debugGetChar(m_lPosn));
  ...
  case Op_##mp_name##_F:                       \
    OP_IMPL_ONE_WIDTH_F(mp_size, is##mp_name);
      ...
      case Op_Capture_B: {  // nNth
        ...
        auto const fHasCapture =
            m_pIContext->GetCapture(nCapture, &lCaptureStart, &lCaptureEnd);

        if (fHasCapture) {
          cpush(Control_Capture, fetchInt(1), lCaptureStart, lCaptureEnd);
        } else {
...
#if DEBUG_EXEC
        StdOutPrintf("\tCapture[%d]=%d..%d <", fetchInt(1), m_lPosn, lEnd);
...
#if DEBUG_EXEC
        StdOutPrintf("\tCapture[%d]=%d..%d<", fetchInt(1), lStart, m_lPosn);
      ...
      case Op_CategoryGt_F:
        ...
        case_Op_OneWidth_BF(CharEq_Ci, 2) case_Op_OneWidth_BF(CharEq_Cs, 2)
            case_Op_OneWidth_BF(CharNe_Ci, 2) case_Op_OneWidth_BF(CharNe_Cs, 2)

                case_Op_OneWidth_BF(CharSetEq, 2)
                    case_Op_OneWidth_BF(CharSetNe, 2)

            // [E]
            case Op_EndOfLine : if (isEndOfLine()) {
          m_nPc += 1;
          break;
        }
        return false;
      ...
      case Op_PushPosn:
        ...
        // [R]
        case_Op_OneWidth_BF(RangeEq_Ci, 3) case_Op_OneWidth_BF(RangeEq_Cs, 3)
...
#define case_Op_Repeat_OneWidth_BF(mp_class)                   \
  case Op_Repeat##mp_class##Eq_B:                              \
    REPEAT_BACKWARD(2, m_pIContext->Is##mp_class(wchSource));  \
  case Op_Repeat##mp_class##Ne_B:                              \
    REPEAT_BACKWARD(2, !m_pIContext->Is##mp_class(wchSource)); \
  case Op_Repeat##mp_class##Eq_F:                              \
    REPEAT_FORWARD(2, m_pIContext->Is##mp_class(wchSource));   \
  case Op_Repeat##mp_class##Ne_F:                              \
    REPEAT_FORWARD(2, !m_pIContext->Is##mp_class(wchSource));

        case_Op_Repeat_Unit_CBF(Char, Eq, charEq) case_Op_Repeat_Unit_CBF(
            Char, Ne, !charEq)

            case_Op_Repeat_Unit_CBF(Range, Eq, rangeEq) case_Op_Repeat_Unit_CBF(
                Range, Ne, !rangeEq)

                case_Op_Repeat_CharSet_BF(
                    Eq, charSetEq) case_Op_Repeat_CharSet_BF(Ne, !charSetEq)
...
class CiCompare {
 public:
  static bool BackwardFindChar(IMatchContext* pIContext,
                               char16 wchLast,
                               Posn* inout_lPosn,
                               Posn lStop) {
  ...
  static char16 Fold(IMatchContext* pIContext, char16 wch) {
    return pIContext->CharUpcase(wch);
  }

  static bool ForwardFindChar(IMatchContext* pIContext,
                              char16 wchLast,
                              Posn* inout_lPosn,
                              Posn lStop) {
  ...
  static bool StringEq(IMatchContext* pIContext,
                       const char16* pwch,
                       int cwch,
                       Posn lPosn) {
...
class CsCompare {
 public:
  ...
  static char16 Fold(IMatchContext*, char16 wch) { return wch; }
...
class CharScanner_ : public CharScanner {
 public:
  bool ScanBackward(IMatchContext* pIContext,
                    Posn* inout_lPosn,
                    Posn lScanStop) const {
    return Case_::BackwardFindChar(pIContext, getChar(), inout_lPosn,
                                   lScanStop);
  }

  bool ScanForward(IMatchContext* pIContext,
                   Posn* inout_lPosn,
                   Posn lScanStop) const {
    return Case_::ForwardFindChar(pIContext, getChar(), inout_lPosn, lScanStop);
  }
};
 ...
 public:
  bool ScanBackward(IMatchContext* pIContext,
    ...
    while (lPosn >= lLoopStop) {
      auto const fFound =
          Case_::BackwardFindChar(pIContext, wchLast, &lPosn, lLoopStop);
      if (!fFound) {
        return false;
      }
      ...
      auto const fEqual =
          Case_::StringEq(pIContext, getString(), m_cwch, lNextPosn);
  ...
  bool ScanForward(IMatchContext* pIContext,
    ...
    while (lPosn <= lLoopMax) {
      auto const fFound =
          Case_::ForwardFindChar(pIContext, wchFirst, &lPosn, lScanStop);
      if (!fFound) {
        return false;
      }
      ...
      auto const fEqual =
          Case_::StringEq(pIContext, getString(), m_cwch, lPosn);
      ...
      switch (pScanner->GetOp()) {
        case Op_AfterNewline:
          ...
          if (isBackward()) {
            for (Posn lPosn = m_lPosn; lPosn >= m_lScanStop; lPosn -= 1) {
              auto const fFound =
                  m_pIContext->BackwardFindCharCs(Newline, &lPosn, m_lScanStop);
              if (!fFound) {
                return false;
              }
            ...
            for (Posn lPosn = m_lPosn; lPosn <= m_lScanStop; lPosn += 1) {
              auto const fFound =
                  m_pIContext->ForwardFindCharCs(Newline, &lPosn, m_lScanStop);
              if (!fFound) {
                return false;
              }
...
bool RegexObj::NextMatch(Regex::IMatchContext* pIContext) const {
  ...
  Engine oContext(pIContext, GetCodeStart(), GetScanner(), m_nMinLen, fBackward,
