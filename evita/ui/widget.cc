...
class Widget::HitTestResult {
  ...
  public: HitTestResult(Widget* widget, const Point& local_point);
...
Widget::HitTestResult::HitTestResult(Widget* widget, const Point& local_point)
    : local_point_(local_point), widget_(widget) {
}
...
Widget::HitTestResult Widget::HitTest(const Point& local_point) const {
  ...
  for (auto runner = last_child(); runner;
    ...
    auto const child_point = local_point.Offset(-child->bounds().left,
                                                -child->bounds().top);
    if (child->bounds().Contains(child_point)) {
      auto const result = child->HitTest(child_point);
      return result ? result : HitTestResult(const_cast<Widget*>(child),
                                             child_point);
    }
  }
...
Widget::HitTestResult Widget::HitTestForMouseEventTarget(
  ...
  return result ? result : HitTestResult(const_cast<Widget*>(this), point);
}
