...
void StatusBar::Realize(HWND hwndParent) {
  DCHECK(!hwnd_);
  hwnd_ = ::CreateWindowEx(0, STATUSCLASSNAMEW, nullptr, WS_CHILD | WS_VISIBLE,
                           0, 0, 0, 0, hwndParent,
                           reinterpret_cast<HMENU>(this), g_hInstance, nullptr);
  WIN32_VERIFY(hwnd_);
  RECT rect;
...
void StatusBar::Set(const std::vector<base::string16>& new_texts) {
  ...
  for (auto index = 0u; index < num_columns; ++index) {
    column_widths_[index] =
        std::max(PartWidth(new_texts[index]), column_widths_[index]);
  }
  ...
  for (auto index = 0u; index < num_columns; ++index) {
    ...
    ::SendMessage(hwnd_, SB_SETTEXT, static_cast<WPARAM>(index | SBT_NOBORDERS),
...
void StatusBar::SetPart(size_t part, const base::string16& new_text) {
  ...
  ::SendMessage(hwnd_, SB_SETTEXT, static_cast<WPARAM>(part | SBT_NOBORDERS),
                reinterpret_cast<LPARAM>(new_text.c_str()));
