...
[
    ...
    'viewport',
    // W
    'waitable', 'whitespace', 'yosi', 'yosin',
].forEach(function(word) { SpellChecker.keywords.add(word); });
...
[
    ...
    // C++
    'std::all_of', 'std::any_of', 'std::array', 'std::begin', 'std::bitset',
    'std::boolalpha', 'std::count', 'std::count_if', 'std::deque', 'std::end',
    'std::endl', 'std::find', 'std::find_if', 'std::for_each', 'std::tie',
    'std::hash', 'std::hex', 'std::ignore', 'std::lower_bound',
    'std::make_pair', 'std::make_tuple', 'std::make_unique', 'std::mismatch',
    'std::next', 'std::none_of', 'std::noboolalpha', 'std::ostream',
    'std::ostringstream', 'std::pair', 'std::prev', 'std::queue', 'std::stack',
    ...
    'std::make_unsigned', 'std::rank', 'std::remove_all_extents',
    'std::remove_constant', 'std::remove_cv', 'std::remove_extent',
    'std::remove_pointer', 'std::remove_reference', 'std::remove_volatile',
    'std::result_of', 'std::true_type', 'std::underlying_type',
    ...
    'v8::FunctionCallback', 'v8::FunctionTemplate', 'v8::Local',
    'v8::HandleScope', 'v8::Isolate', 'v8::Local', 'v8::Object',
    'v8::ObjectTemplate', 'v8::Persistent', 'v8::Promise', 'v8::Script',
...
(function() {
  ...
  function sourcePath(document) {
    ...
    const srcIndex = path.findIndex(
        (element, index) => {return index > 0 && element === 'src'});
    if (srcIndex < 0)
      return path;
