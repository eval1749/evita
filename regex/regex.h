...
class IEnvironment {
 public:
  virtual char16 CharUpcase(char16) const;

 public:
  virtual char16 CharDowncase(char16) const;

 public:
  virtual bool IsBothCase(char16) const;

 public:
  virtual bool IsAsciiDigitChar(char16 wch) const {
    return Regex::IsAsciiDigitChar(wch);
  }

 public:
  virtual bool IsAsciiSpaceChar(char16 wch) const {
    return Regex::IsAsciiSpaceChar(wch);
  }

 public:
  virtual bool IsAsciiWordChar(char16 wch) const {
    return Regex::IsAsciiWordChar(wch);
  }

 public:
  virtual bool IsUnicodeDigitChar(char16 wch) const {
    return Regex::IsUnicodeDigitChar(wch);
  }

 public:
  virtual bool IsUnicodeSpaceChar(char16 wch) const {
    return Regex::IsUnicodeSpaceChar(wch);
  }

 public:
  virtual bool IsUnicodeWordChar(char16 wch) const {
...
interface IMatchContext : IEnvironment {
  ...
  // [R]
  virtual void ResetCapture(int index) = 0;
