...
(function() {
  ...
  function copyToClipboardCommand() { copyToClipboard(this.selection.range); }
  ...
  Editor.bindKey(
      TextWindow, 'Ctrl+Backspace',
      /**
       * Backward delete word
       * @param {number=} count
       * @this {!TextWindow}
       */
      function(count = 1) { this.selection.range.delete(Unit.WORD, -count); });

  Editor.bindKey(TextWindow, 'Ctrl+C', copyToClipboardCommand);

  Editor.bindKey(
      TextWindow, 'Ctrl+Delete',
      /**
       * Forward delete word
       * @param {number=} count
       * @this {!TextWindow}
  ...
  Editor.bindKey(
      TextWindow, 'Ctrl+End',
      function() { this.selection.endKey(Unit.DOCUMENT); },
  ...
  Editor.bindKey(
      TextWindow, 'Ctrl+Home',
      function() { this.selection.homeKey(Unit.DOCUMENT); },
  ...
  Editor.bindKey(
       ...
       * @this {!TextWindow}
       */
      function() { this.makeSelectionVisible(); });
  ...
  Editor.bindKey(
      ...
      function(arg) {
        ...
        Editor
            .messageBox(
                this, Editor.localizeText(
                          Strings.IDS_ASK_RELOAD, {name: document.name}),
  ...
  Editor.bindKey(
      TextWindow, 'End',
      function() { this.selection.endKey(Unit.WINDOW_LINE); },
  ...
  Editor.bindKey(
      ...
      function(count = 1) {
        if (count <= 0)
          return;
        var selection = this.selection;
        var leadingWhitespaces = (new TextRange(selection.range))
                                     .startOf(Unit.LINE)
                                     .moveEndWhile(' \t')
                                     .text;
  ...
  Editor.bindKey(
      TextWindow, 'Home',
      function() { this.selection.homeKey(Unit.WINDOW_LINE); },
  ...
  Editor.bindKey(
      TextWindow, 'Shift+Home',
      function() { this.selection.homeKey(Unit.WINDOW_LINE, Alter.EXTEND); },
  ...
  /** @const @type {number} */
  // TODO(eval1749): We should get |Indent|/|Outend| tab width from another
  // place.
  var TAB_WIDTH = 4;

  Editor.bindKey(
      TextWindow, 'Shift+Tab',
